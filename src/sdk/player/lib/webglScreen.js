export default class WebglScreen {
  ...
  /**
    ...
    renderImg(width, height, data) {
        if(this.type == 1){
            this._renderImg(width, height, data)
        }else if(this.type == 2){
     ...
     * @param {number} height 高度
     * @param {字节数组} data 数据源
     * @param {int} speed 每秒显示多少帧
     */
    renderMoreImg(width, height, data, speed){
        ...
        let interval = setInterval(() => {
            this.renderImg(width, height, data.subarray(curFrame*frameItemSize, (curFrame+1)*frameItemSize))
            curFrame++;
     ...
     */
    getShoot() {
        let data = this.ctx.canvas.toDataURL("image/png"); // 将转码的base64 传出去展示出来
