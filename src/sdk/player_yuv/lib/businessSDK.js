...
WebSocketSDK.prototype = {
		...
		if(event.data instanceof ArrayBuffer){//二进制
			...
			if(this._isEmpty(this._receiveBusinessCB) == false){
				...
				if(funName == 'Init'){
					...
					eventType = 1;
					status_code = 2;;
				}else if(funName == 'StartPlayInPos' || funName == 'StartPlayByRtpId'){
					eventType = 0;
					status_code = data.result >=0 ? 1 : 0;
				...
				this._receiveBusinessCB(eventType, status_code, screenIndex);
			}
		}		
		event.length = 0;
		event = null;		
	...
	groupStartCommand: function(groupId, cmd) {
		var data = {
			...
			"groupId": groupId, //组ID
			"cmd": JSON.stringify(cmd) //cmd 需要记录的指令（json,如[{"pos":"0","videoRtpId":"101","audioRtpId":"102"}]）
		  };
		  this.publishBase(data);
	},
