...
var decoderPlayer = {
	...
	_refreshOperate : function(screenIndex){
		...
		if(screenInfo.business == BUSINESS_NONE || screenInfo.isFull){
	...
	_initOperate(screenIndex){
		...
		if(screenInfo.screenType == 1){//点播屏的按钮操作栏
			...
			//初始化声音条
			screenInfo.volume = new Slider(operateDiv.querySelector('.volume-progress'), 50, {circleSize: 12});
			...
			var innerHTML =
					...
					"		<td style=\"width:100%\"></td>" +
					"		<td><img style=\"width:20px; height:20px; padding: 0px 0px 0px 5px; cursor: pointer;\"" + jsOperate5 + "></img></td>" +
					"		<td><img style=\"width:20px; height:20px; padding: 0px 5px 0px 5px; cursor: pointer;\"" + jsOperate4 + "></img></td>" +
					"	</tr>" +
					"</table>";
			operateDiv.innerHTML = innerHTML;
			//初始化声音条
	...
	_refreshCanvas : function(screenIndex){
		...
		if(screenInfo.business == BUSINESS_NONE){
			...
			if(canvasDiv.style.display != "block"){
				...
				//创建canvas
				var glScreen = new WebglScreen(canvasDiv);
	...
	/*对外接口*********************************************************************/
    //初始化
	init : function(divId, containerWidth, containerHeight){
		// 清除已保存分屏信息
