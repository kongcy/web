    ...
    const getMedia = function() {
        ...
        // return navigator.mediaDevices.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia || navigator.getUserMedia;
        //0623 dj修改  360兼容模式 navigator.mediaDevices==underfind
        return (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia || navigator.getUserMedia;
    ...
    Recorder.throwError = function(message) {
        ...
        //是否支持录音   
    Recorder.canRecording = (getMedia() != null);
    //获取录音机  
    Recorder.get = function(callback, config) {
        if (callback) {
            if (getMedia()) {
                xtxk.media.getUserMedia({ audio: true } //只启用音频  A
                    ,
                    ...
                    function(error) {
                        ...
                        switch (error.code || error.name) {
                            ...
                            case 'PermissionDeniedError':
                                Recorder.throwError('用户拒绝提供信息。或无法打开麦克风。');
                                break;
                            case 'NotFoundError':
                                Recorder.throwError('无法打开麦克风。');
                                break;
    ...
    export default { Recorder, getMedia }