<template>
    <div>
        <el-dialog :visible.sync="isVisible" title="会场转发授权" width="400px" class="custom-dialog" @close="closeDialog">
            <div class="commandListBox">
                ...
                <div class="authBtn" @click="startConferenceAuthorize">开启授权</div>
            </div>
            <p class="authTitle"><span class="authTitle_span">已授权成员：</span><span class="authTitle_btn" v-if="authorizedList.length>0" @click="stopAllConferenceAuthorize">全部关闭</span></p>
            <div class="authorizedListBox">
                <ul>
                    <li v-for='item in authorizedList' :key="item.id">
                        <span class="authorizedName" :title="item.name">{{item.name}}</span><span class="authorizedBtn" @click="stopConferenceAuthorize(item)">关闭</span>
                    </li>
                </ul>
            </div>
        </el-dialog>
...
<script>
export default {
    name: "ConferenceForwardAuthorizeDialog",
    data() {
        return {
            isVisible:false,
            treeData:[],
    ...
    methods:{
        ...
        startConferenceAuthorize(){
            ...
            this.apiSDK.startMeetingTransmitAuth(this.affairId, users);
        },
        closeDialog(){
            this.isVisible = false;
        },
        // 停止会场转发授权
        stopConferenceAuthorize(item){
            let users = [];
            users.push(item.id);
            this.apiSDK.stopMeetingTransmitAuth(this.affairId, users);
        },
        // 全部停止会场转发授权
        stopAllConferenceAuthorize(item){
            let users = [];
