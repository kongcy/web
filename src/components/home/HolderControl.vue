<template>
  <div class="holder">
    ...
    <div class="holder-content">
        ...
        <div class="direction_right">
          <div class="adjust">
            <p v-for="(item, index) in adjustData" :key="index">
              <span class="adjustAdd" @mousedown="addReduce(item, true, 'mousedown')"  @mouseup="addReduce(item, true, 'mouseup')"></span>
              <span>{{ item }}</span>
              <span class="adjustReduce" @mousedown="addReduce(item, false, 'mousedown')" @mouseup="addReduce(item, false, 'mouseup')"></span>
            </p>
          </div>
      ...
      <div class="holder-btn">
        <div class="holdControlBox unbindLock">
          <div v-if="!display.unbindLock && this.apiSDK.config.platformVersion == 0" @click="publishTakeoverYT">
            <i class="holdControlbtn icon-unbingLock-start" title="开始接管"></i>
            <div>接管</div>
          </div>
          <div v-if="display.unbindLock && this.apiSDK.config.platformVersion == 0" @click="publishTakeoverStopYT">
     ...
     <div class="holder-PresetGroup">
       ...
       <el-scrollbar class="holderTree holderControl hiddenXScroll">
        ...
        <div class="rightBottom"  v-if="!isNVRChannel">
          <el-button type="primary" size="small" v-for="item in bottomData" :key="item.title" @click="clickBottom(item)" >{{item.title}}</el-button>
    ...
    <prepoints-manage-dialog
      ref="prepointsManageDialog"
      @close="getPrepareData()"
    />
  ...
  methods: {
    ...
    stopDirectionCtl(event) {
      if (!this.isNVRChannel) {
        this.apiSDK.publishStopYTDirectionCtl(
          this.resourceId,
          this.resourceCh,
          event.direction,
          event.direction2
        ...
        this.apiSDK.publishStopYTNVRDirectionCtl(
          this.resourceId,
          this.channelEncoderSIPID,
          this.channel
        );
      }
    },
    ...
    clickDirection(event) {
      if (this.isNVRChannel) {
        this.apiSDK.publishStartYTNVRDirectionCtl(
          this.resourceId,
          event.direction,
          this.sliderValue,
          this.channel,
      ...
      if (event.direction == "") {
        ...
        if (!this.isNVRChannel) {
          this.apiSDK.publishStartYTDirectionCtl(
            this.resourceId,
            this.resourceCh,
            event.direction,
            event.direction2,
    ...
    publishTakeoverYT() {
      ...
      this.apiSDK.publishStartYTTakeOverCtl(
        this.resourceId,
        this.resourceCh,
        function (obj) {}
      );
    ...
    publishTakeoverStopYT() {
      ...
      this.apiSDK.publishStopYTTakeOverCtl(
        this.resourceId,
        this.resourceCh,
        function (obj) {}
      );
    ...
    renderContent(h, { node, data, store }) {
      if (this.isNVRChannel) {
        return (
          <span class={"prepare-tree-node " + data.nodeStatus}>
        ...
        return (
          <span class={"prepare-tree-node " + data.nodeStatus}>
            ...
            <span class="operateDot">
              <span
                class="edit_icon"
                on-click={() => this.updateGroup(data)}
              ></span>
              <span
                class="del_icon"
                on-click={() => this.deleteGroup(data)}
              ></span>
            </span>
          </span>
        );
    ...
    deleteGroup(data) {
      let that = this;
      let groupId = data.id;
      this.$confirm('是否确认删除该预置点('+data.name+')?', '提示', {
    ...
    updateOperate(data) {
      ...
      this.$prompt("请输入预置点名称", "编辑预置点", {
        customClass:'customLeft fixedRB',
        confirmButtonText: "保存",
        cancelButtonText: "取消",
        inputPattern: /^[\u4e00-\u9fa5A-Za-z0-9._-]{1,20}$/,
    ...
    setOperate(data) {
      ...
      this.apiSDK.setDefaultUserYTPoint(this.resourceId,this.resourceCh,data.id,isDefault,function (obj) {
          if (obj && obj.Ret == 0) {
            that.getPrepareData();
            that.$message({
              message: "设置成功",
    ...
    removeAllClass() {
      ...
      const selectNodeDot = document.querySelectorAll(
        ".holderControl .el-tree-node__content span.operateGroup"
      );
    ...
    clickBottom(data) {
      switch (data.class) {
        case "prepare":
          this.$refs.prepointsManageDialog.showDialog(
            this.resourceId,
            this.resourceCh
          );
          break;
        ...
        case "prepareAdd":
          ...
          break;
        case "cruise":
          this.$refs.cruseManageDialog.showDialog(
            this.resourceId,
            this.resourceCh
          );
          break;
...
.el-tree-node__content:hover .operateDot .edit_icon:hover,
.el-tree-node__content:hover .operateGroup .edit_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/node-update_active.png");
}
...
.is-current .el-tree-node__content:hover .operateGroup .edit_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/node-update_active.png");
}
...
.el-tree-node__content:hover .operateGroup .curr_icon:hover{
  visibility: visible;
  background: url("../../../static/holderControl/skipPrepare_active.png");
}
...
.is-current .el-tree-node__content:hover .operateGroup .curr_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/skipPrepare_active.png");
}
...
.el-tree-node__content:hover .operateGroup .setDefault_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/acquiesceIn_active.png");
}
...
.is-current .el-tree-node__content:hover .operateGroup .setDefault_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/acquiesceIn_active.png");
}
...
.el-tree-node__content:hover .operateDot .del_icon:hover,
.el-tree-node__content:hover .operateGroup .del_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/delPrepare_active.png");
}
...
.is-current .el-tree-node__content:hover .operateGroup .del_icon:hover {
  visibility: visible;
  background: url("../../../static/holderControl/delPrepare_active.png");
}
...
.icon-unbingLock-start {
  background: url(../../../static/holderControl/bindLock.png) center center
    no-repeat;
  background-size: 32px 32px;
}
.icon-unbingLock-start:active {
  background: url(../../../static/holderControl/bindLock_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-unbingLock-start{
  background: url(../../../static/holderControl/bindLock_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
...
.icon-unbingLock-close {
  background: url(../../../static/holderControl/unbindLock1.png) center center
    no-repeat;
  background-size: 32px 32px;
}
.icon-unbingLock-close:active {
  background: url(../../../static/holderControl/unbindLock1_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-unbingLock-close{
  background: url(../../../static/holderControl/unbindLock1_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
...
.icon-windshieldWiper-start {
  background: url(../../../static/holderControl/windshieldWiper_start.png)
    center center no-repeat;
  background-size: 32px 32px;
}
.icon-windshieldWiper-start:active {
  background: url(../../../static/holderControl/windshieldWiper_start_active.png)
    center center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-windshieldWiper-start{
  background: url(../../../static/holderControl/windshieldWiper_start_active.png)
    center no-repeat;
  background-size: 32px 32px;
}
...
.icon-windshieldWiper-close {
  background: url(../../../static/holderControl/windshieldWiper_close.png)
    center center no-repeat;
  background-size: 32px 32px;
}
.icon-windshieldWiper-close:active {
  background: url(../../../static/holderControl/windshieldWiper_close_active.png)
    center center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-windshieldWiper-close{
  background: url(../../../static/holderControl/windshieldWiper_close_active.png)
    center no-repeat;
  background-size: 32px 32px;
}
/* 打开加热 */
.icon-heat-start {
  background: url(../../../static/holderControl/heat_start.png) center center
    no-repeat;
  background-size: 32px 32px;
}
.icon-heat-start:active {
  background: url(../../../static/holderControl/heat_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-heat-start{
  background: url(../../../static/holderControl/heat_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
...
.icon-heat-close:active {
  background: url(../../../static/holderControl/heat_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-heat-close{
  background: url(../../../static/holderControl/heat_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
/* 打开3d */
.icon-3d-start {
  background: url(../../../static/holderControl/_3d_start.png) center center
    no-repeat;
  background-size: 32px 32px;
}
.icon-3d-start:active {
  background: url(../../../static/holderControl/_3d_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-3d-start{
  background: url(../../../static/holderControl/_3d_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
...
.icon-3d-close:active {
  background: url(../../../static/holderControl/_3d_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
.holdControlBox>div:hover .icon-3d-close{
  background: url(../../../static/holderControl/_3d_start_active.png) center
    center no-repeat;
  background-size: 32px 32px;
}
...
.holder-btn .lock01 {
  width: 20px;
  height: 20px;
  background: url(../../../static/holderControl/unbindLock.png) center center
    no-repeat;
  position: absolute;
  left: 0;
  padding: 0;
...
.holder-btn .wiper01 {
  width: 20px;
  height: 20px;
  background: url(../../../static/holderControl/windshieldWiper.png) center
    center no-repeat;
  position: absolute;
  left: 0;
  bottom: 0;
...
.holder-btn .heat01 {
  width: 20px;
  height: 20px;
  background: url(../../../static/holderControl/heat.png) center center
    no-repeat;
  position: absolute;
  right: 0;
  bottom: 0;
...
.direction .up-left:hover > span {
  background: url("../../../static/holderControl/up-left_hover.png");
}
.direction .up-left:active > span {
...
.direction .up-centre:hover > span {
  background: url("../../../static/holderControl/up-centre_hover.png");
}
.direction .up-centre:active > span {
...
.direction .up-right:hover > span {
  background: url("../../../static/holderControl/up-right_hover.png");
}
.direction .up-right:active > span {
...
.direction .centre-left:hover > span {
  background: url("../../../static/holderControl/centre-left_hover.png");
}
.direction .centre-left:active > span {
...
.direction .centre-centre:hover > span {
  background: url("../../../static/holderControl/centre-centre_hover.png");
}
.direction .centre-centre:active> span {
...
.direction .centre-right:hover > span {
  background: url("../../../static/holderControl/centre-right_hover.png");
}
.direction .centre-right:active > span {
...
.direction .below-left:hover > span {
  background: url("../../../static/holderControl/below-left_hover.png");
}
.direction .below-left:active > span {
...
.direction .below-centre:hover > span {
  background: url("../../../static/holderControl/below-centre_hover.png");
}
.direction .below-centre:active > span {
...
.direction .below-right:hover > span {
  background: url("../../../static/holderControl/below-right_hover.png");
}
.direction .below-right:active > span {
...
.adjust .adjustAdd:hover {
  background: url("../../../static/holderControl/add_hover.png") no-repeat
    center;
  background-size: 100% 100%;
}
.adjust .adjustAdd:active {
  background: url("../../../static/holderControl/add_active.png") no-repeat
    center;
  background-size: 100% 100%;
}
.adjust .adjustReduce {
...
.adjust .adjustReduce:hover {
  background: url("../../../static/holderControl/reduce_hover.png") no-repeat
    center;
  background-size: 100% 100%;
}
.adjust .adjustReduce:active {
  background: url("../../../static/holderControl/reduce_active.png") no-repeat
    center;
  background-size: 100% 100%;
}
...
.slider /deep/ .el-slider__button {
  ...
  border-radius: 0;
  background: url(../../../static/holderControl/slider-pointer.png) no-repeat
    center;
  background-size: 100% 100%;
  margin-top: -6px;
}
.slider /deep/ .el-slider__button:hover {
  width: 24px;
  height: 24px;
  background: url(../../../static/holderControl/slider-pointer_active.png)
    no-repeat center;
  background-size: 100% 100%;
  transform: scale(1);
}
...
.rightBottom {
  ...
  height: 54px;
  background:url(../../../static/main/screen/resource_bottom_bg.png);
