<template>
  <div id="divResourceArea" width="414px">
    ...
    <!-- 资源操作区域 -->
    <div id="divResourceOperate" v-if="activetabName!='Monitor'">
      ...
      <input type="button" class="video-conferecing" title="视频会议"  @click="startTemproaryVideo('conferenceScene')"  v-if="activetabName!='Monitor'">
      <input v-if="(apiSDK.config.version === apiSDK.enumSDKVersion.SDKVersion5)&&activetabName!='Monitor'" type="button" class="video-voice" title="语音会议"  @click="startTemproaryVideo('temproaryConference', 'Audio')">
      <!-- <input type="button" class="video-command" title="视频指挥"  @click="startTemproaryVideo('temproaryCommand')"> -->
    </div>

   
    <!-- 资源显示区域 -->
    <div class="tabWrap" v-if="activetabName=='Monitor'" :style="{height:activetabName!='Monitor'?'calc(100% - 40px)':'100%'}">
    ...
    <div class="tabWrap"  v-else :style="{height:activetabName!='Monitor'?'calc(100% - 40px)':'100%'}">
