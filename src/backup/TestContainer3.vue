<template>
    ...
    <div id="canvasContainer" style="width:1200px;height: 800px;">
    </div>
  </div> 
</template>
<script>
import {businessSDKC, businessSDKD} from '@/sdk/player/businessSDK'
import {webglPlayer} from '@/sdk/player/webglPlayer'
...
export default {
  ...
  mounted(){
    ...
    businessSDKD.setReceiveMediaDataCallback(res => {
      console.log("size:"+res.byteLength)
      data_uint8 = new Uint8ClampedArray(res, 10);//一帧yuv数据
      headParams16 = new Int16Array(res, 0, 10);//头部参数
  ...
  methods: {
    initLayout(){
      webglPlayer.init("canvasContainer")
    },
    initSocket(){
      this.initLayout();
    ...
    startPlaySocket(count){
      ...
      for (let index = 0; index < this.screenCount; index++) {
        ...
        if(this.screenCount == 1) businessSDKC.startPlay(screenIndex, 1920, 1080);
        else if(this.screenCount == 4) businessSDKC.startPlay(screenIndex, 704, 576);
        else if(this.screenCount == 9) businessSDKC.startPlay(screenIndex, 352, 288);
        webglPlayer.startShow(screenIndex, "User");        
      }
    },
