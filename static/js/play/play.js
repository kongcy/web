...
$.fn.extend({
    video:function(wssConn,cid,key,isAudio,videoChangerCallBack) {
        ...
        topObj._toPlay = function(data) {
            ...
            if (topObj.isIEState) {
				...
				if(!this.isAudio&&canvas){
					...
					console.log('7-jsmpeg---'+topObj.key+'解码准备!  '+ new Date().formatDate('HH:mm:ss.SS')); 
				}
		...
		function sendMessage(data){
			var ret = false;
			if(wssConn.wsObj.wss.readyState === 1){
				wssConn.wsObj.wss.send(JSON.stringify(data));
				ret = true;
			}
			else {
				console.log('服务器'+topObj.serverUrl+'无法连接,发送失败!')
			}
			return ret;
		}
		// 暂停/播放切换
